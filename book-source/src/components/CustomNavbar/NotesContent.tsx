...
const NotesContent: React.FC = () => {
  ...
  useEffect(() => {
    setView(initialView);
    // If opening in 'add' mode with selected text, prefill the content field
    if (initialView === 'add' && selectedText) {
      // Put the full selected text in the content field
      setNoteContent(selectedText);

      // Generate a shorter name from the selected text (first few words)
      const words = selectedText.trim().split(/\s+/);
      const autoGeneratedName =
        words.length > 5 ? words.slice(0, 5).join(' ') + '...' : selectedText;
      setNoteName(autoGeneratedName);
    }
